...
*⚠️The plugin communicates data back to qBittorrent via stdout and that means you must NOT print debug/error messages to stdout under any circumstances. You can print the debug/error messages to stderr instead.*
...
You'll also need [the other files](https://github.com/qbittorrent/qBittorrent/tree/master/src/searchengine/nova3) for the project
...
* **[ADVISED METHOD]** [html.parser](https://docs.python.org/3/library/html.parser.html): Builtin python parser which replaces deprecated SGMLParser. Mostly similar to the SGMLParser
...
Note that size is provided in bytes.
...
* `link` => A string corresponding to the download link (the .torrent file or magnet link)
...
Do not hesitate to use the [official search engine plugins](https://github.com/qbittorrent/search-plugins/tree/master/nova3/engines) as an example.
...
3. Click on "Install a new one" at the bottom and select your `*.py` python script on your filesystem. \
...
* Some search engines do not provide all the information required by `prettyPrinter()`. If it is the case, set `-1` as value for the given key (i.e.: `torrent_info['seeds'] = -1`)
* Plugins packaged in a Python archive/package are no longer directly installable since qBittorrent v2.0.0. You must provide qBittorrent with the Python file.
